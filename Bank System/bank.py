...
class Bank:
    ...
    def _load_clients(self) -> None:
        try:
            ...
            for row in cursor.fetchall():
                ...
                cursor.execute(
                    "SELECT date, count FROM withdrawal_counts WHERE login = ?",
                    (login,),
                )
                client.withdrawal_cnt = {
                    date: count for date, count in cursor.fetchall()
                }
    ...
    def registered_new_user(
        ...
        try:
            ...
            account_nbrs = [
                int(account) for account, in cursor.fetchall() if account.isdigit()
            ]
